# FastAPI protobuf 직렬화 데모 (1_bookstore)
...
gRPC 서버/포트는 필요 없습니다. REST 로 JSON 을 받아 protobuf 메시지로 바꾸고
바이너리로 직렬화한 뒤, 그 바이너리를 다시 메시지로 되돌립니다.
...
fastapi/
├── app/
│   ├── proto/         # 컴파일된 protobuf 코드 (book_pb2.py)
│   ├── serializer.py  # proto 변환 + 직렬화/역직렬화 핵심
