# FastAPI HTTP→gRPC 게이트웨이 (12_서버의정보받아오기-리플렉션)

이 예제의 gRPC 서버를 백엔드로 두고, FastAPI 가 REST 엔드포인트를 노출하는
게이트웨이(BFF) 샘플입니다. 핵심은 **gRPC 서버 리플렉션**으로, 클라이언트가
.proto 파일을 미리 갖고 있지 않아도 서버에 직접 물어 어떤 서비스를 노출하는지
동적으로 발견하는 것입니다.
...
├── app/
│   ├── proto/         # 컴파일된 gRPC 코드
│   ├── grpc_client.py # Echo 스텁 + 리플렉션 스텁
...
# 1) 백엔드 gRPC 서버 기동 (상위 폴더의 예제 서버, 포트 50062 로 수정 후)
...
`reflection_example.EchoService` 를 발견하는지 end-to-end 로 검증합니다.
