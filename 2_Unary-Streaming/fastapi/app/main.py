...
REST 엔드포인트(POST /hello)를 노출하고, 내부적으로 gRPC 백엔드의
`Greeter.SayHello` 를 호출한다. gRPC 상태코드를 HTTP 상태코드로 매핑해
외부에는 표준 HTTP 의미로 응답한다.
"""
...
from app.schemas import HelloRequest, HelloResponse
...
app = FastAPI(title="gRPC Unary 게이트웨이", lifespan=lifespan)
...
def hello(req: HelloRequest) -> HelloResponse:
    ...
    try:
        message = client.say_hello(req.name)
    except grpc.RpcError as exc:
        # 백엔드가 에러를 던지면 gRPC 상태코드를 HTTP 코드로 매핑한다.
    ...
    return HelloResponse(message=message, elapsed_ms=round(elapsed_ms, 2))
