# Django HTTP→gRPC 게이트웨이 (9_인터셉터)

이 예제의 gRPC 서버(`EchoService`)를 백엔드로 두고, Django 가 REST 엔드포인트를
노출하는 게이트웨이 샘플입니다. 핵심은 **gRPC 클라이언트 인터셉터**
...
├── gateway/           # app: views, urls, grpc_client(인터셉터), proto/
├── tests/             # 통합 테스트
...
# 1) 백엔드 gRPC 서버 기동 (상위 폴더 예제 서버, 포트 50059)
