...
from grpc_health.v1 import health_pb2, health_pb2_grpc  # 표준 헬스체크 프로토콜의 메시지(health_pb2)와 서버 등록 함수(health_pb2_grpc) 모듈
from grpc_health.v1.health import HealthServicer  # 헬스체크 서비스의 기본 인터페이스(부모 클래스)
from grpc_health.v1.health_pb2 import HealthCheckResponse  # 상태 enum(SERVING 등)과 응답 메시지 타입


class HealthCheck(HealthServicer):  # 표준 HealthServicer를 상속받아 헬스체크 로직을 직접 구현하는 클래스
    ...
    def Check(self, request, context):  # 클라이언트의 1회성 상태 조회 요청을 처리하는 메서드
        ...
        return health_pb2.HealthCheckResponse(status=status)  # 조회한 상태값을 담은 응답 메시지를 반환
    ...
    def Watch(self, request, context):  # 클라이언트에게 상태를 스트림으로 계속 전송하는 메서드
        while True:  # 연결이 유지되는 동안 무한 반복하며 상태를 주기적으로 전송
            ...
            yield health_pb2.HealthCheckResponse(status=status)  # 상태값을 담은 응답을 스트림으로 하나씩 전송(yield)
