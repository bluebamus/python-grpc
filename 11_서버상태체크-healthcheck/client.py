...
from grpc_health.v1 import health_pb2, health_pb2_grpc  # gRPC 표준 헬스체크 프로토콜의 메시지(health_pb2)와 스텁(health_pb2_grpc) 모듈
...
def run_check():  # 서버 상태를 1회성(unary)으로 조회하는 함수 정의
    with grpc.insecure_channel('localhost:50051') as channel:  # TLS 없이 localhost:50051 서버에 연결되는 채널 생성(with 블록 종료 시 자동 닫힘)
        stub = health_pb2_grpc.HealthStub(channel)  # 채널을 이용해 Health 서비스의 원격 메서드를 호출할 수 있는 스텁 생성
        request = health_pb2.HealthCheckRequest(service='my_service')  # 'my_service' 서비스의 상태를 묻는 요청 메시지 생성
...
def run_watch():  # 서버 상태 변화를 지속적으로(server streaming) 감시하는 함수 정의
    with grpc.insecure_channel('localhost:50051') as channel:  # TLS 없이 localhost:50051 서버에 연결되는 채널 생성(with 블록 종료 시 자동 닫힘)
        stub = health_pb2_grpc.HealthStub(channel)  # 채널을 이용해 Health 서비스의 원격 메서드를 호출할 수 있는 스텁 생성
        responses = stub.Watch(health_pb2.HealthCheckRequest(service='my_service'))  # Watch 메서드를 호출하여 상태 응답을 연속으로 받는 스트림(이터레이터) 획득
