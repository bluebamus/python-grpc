...
from concurrent import \
    futures  # gRPC 서버가 요청을 동시에 처리할 스레드 풀(ThreadPoolExecutor)을 제공
...
from health_check_pb2 import \
    HealthCheckResponse  # 상태 enum(SERVING 등)과 응답 메시지 타입
...
class HealthCheck(health_check_pb2_grpc.healthServicer):  # 생성된 healthServicer를 상속받아 헬스체크 로직을 직접 구현하는 클래스
    def __init__(self):  # 인스턴스 생성 시 서비스별 상태 정보를 초기화
        ...
        self.start_time = time.time()  # 서버 시작 시각을 기록하여 이후 상태 변경에 활용할 수 있도록 함 

    def Check(self, request, context):  # 클라이언트의 1회성 상태 조회 요청을 처리하는 메서드
        current_time = time.time()  # 현재 시각을 가져옴
        if current_time - self.start_time > 10:  # 서버가 시작된 지 10초가 지났다면
            return HealthCheckResponse(status=HealthCheckResponse.NOT_SERVING)  # 상태를 NOT_SERVING(비정상)으로 반환
        status = self.status_map.get(request.service, HealthCheckResponse.SERVICE_UNKNOWN)  # 요청한 서비스의 상태를 조회, 없으면 SERVICE_UNKNOWN 반환
        return HealthCheckResponse(status=status)  # 조회한 상태값을 담은 응답 메시지를 반환

    def Watch(self, request, context):  # 클라이언트에게 상태를 스트림으로 계속 전송하는 메서드
        while True:  # 연결이 유지되는 동안 무한 반복하며 상태를 주기적으로 전송
            current_time = time.time()  # 현재 시각을 가져옴
            if current_time - self.start_time > 10:  # 서버가 시작된 지 10초가 지났다면
                yield HealthCheckResponse(status=HealthCheckResponse.NOT_SERVING)  # 상태를 NOT_SERVING(비정상)으로 스트림으로 전송
            else:
                status = self.status_map.get(request.service, HealthCheckResponse.SERVICE_UNKNOWN)  # 요청한 서비스의 현재 상태를 조회, 없으면 SERVICE_UNKNOWN
                yield HealthCheckResponse(status=status)  # 상태값을 담은 응답을 스트림으로 하나씩 전송(yield)
