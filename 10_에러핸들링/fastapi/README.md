# FastAPI HTTP→gRPC 게이트웨이 (10_에러핸들링)

이 예제의 gRPC 서버를 백엔드로 두고, FastAPI 가 REST 엔드포인트를 노출하는
게이트웨이(BFF) 샘플입니다. 핵심은 **gRPC 에러를 HTTP 의미로 번역**하는 것입니다.
...
├── app/
│   ├── proto/         # 컴파일된 gRPC 코드
│   ├── grpc_client.py # 채널/스텁 + Divide 호출
│   ├── schemas.py     # Pydantic 요청/응답
│   ├── config.py      # 설정(환경변수, 기본 포트 50060)
│   └── main.py        # FastAPI 앱 + gRPC→HTTP 상태코드 매핑 표
├── tests/             # 통합 테스트
...
# 1) 백엔드 gRPC 서버 기동 (상위 폴더의 예제 서버, 포트 50060)
...
통합 테스트는 결정론적 gRPC 서버를 직접 띄워 에러 매핑을 end-to-end 로 검증합니다.
