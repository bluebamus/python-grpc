...
REST 엔드포인트(POST /hello)를 노출하고, 내부적으로 TLS 보안 채널로 gRPC
백엔드를 호출한다. gRPC 상태코드를 HTTP 상태코드로 매핑해 외부에는 표준
HTTP 의미로 응답한다.
"""
...
async def lifespan(app: FastAPI):
    # 앱 기동 시 TLS 채널을 한 번 열고, 종료 시 닫는다.
    client = GrpcClient()
    client.connect()
...
app = FastAPI(title="gRPC TLS 보안 게이트웨이", lifespan=lifespan)
