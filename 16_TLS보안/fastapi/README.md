# FastAPI HTTP→gRPC 게이트웨이 (16_TLS보안)

이 예제의 gRPC 서버를 백엔드로 두고, FastAPI 가 REST 엔드포인트를 노출하는
게이트웨이(BFF) 샘플입니다. 핵심은 게이트웨이가 백엔드 gRPC 서버에
...
├── app/
│   ├── proto/         # 컴파일된 gRPC 코드
│   ├── grpc_client.py # TLS 보안 채널 + 스텁
│   ├── schemas.py     # Pydantic 요청/응답
│   ├── config.py      # 설정(주소, 인증서 경로, 호스트네임 오버라이드)
│   └── main.py        # FastAPI 앱
...
# 1) 백엔드 gRPC 서버 기동 (상위 폴더의 예제 서버, TLS 포트)
...
> 참고: 상위 `server.py` 는 포트 50051 로 띄웁니다. 게이트웨이의 기본 target
> 은 통합 테스트와 동일한 50066 이므로, 직접 연동 시
> `GATEWAY_GRPC_TARGET=localhost:50051` 로 덮어쓰거나 서버 포트를 맞추세요.
...
게이트웨이가 보안 채널로 호출해 정상 응답을 받는지 end-to-end 로 검증합니다.
또한 평문 채널이 거부되는 것을 확인해 'insecure 가 아님'을 분명히 합니다.
