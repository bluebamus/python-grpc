...
FastAPI 샘플과 동일하게, TLS gRPC 서버를 백그라운드로 띄운다. 서버는 상위
폴더(16_TLS보안/)의 기존 자체서명 인증서(server.key/server.crt)로
`add_secure_port` 한다. insecure 포트는 전혀 열지 않으므로, 게이트웨이가
정상 응답을 받으면 그것은 TLS 연결이 성공했다는 증거다.
"""
...
# 배정 포트(이 예제 전용). settings 기본 target 과 동일해야 한다.
GRPC_PORT = 50066

# 상위 폴더의 기존 자체서명 인증서 위치.
#   tests/conftest.py -> parents[0]=tests, [1]=django, [2]=16_TLS보안
